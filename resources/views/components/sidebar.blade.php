<nav class="shrink-0 w-56 py-12 bg-yellow2 flex overflow-y-auto max-mobile:w-14 overflow-x-hidden fixed h-full">
    ...
    <ul class="flex flex-col">
        <li>
            <a href="{{ route('home') }}"
               ...
               title="Vers la page d'accueil">
                ...
                <span class="font-medium max-mobile:hidden">
                    Accueil
                </span>
            </a>
        </li>
        ...
        <li>
            <a href="{{ route('draw') }}"
               class="@if (route('draw') === url()->current() ) bg-yellow @endif flex items-center text-black group px-12 rounded-r-full py-4 max-mobile:p-4 max-mobile:rounded-r"
        ...
        <div class="mt-auto mb-10">
            ...
            <li>
                <a href="{{ route('settings') }}" title="Vers la page des paramètres"
                   class="@if (route('settings') === url()->current()) bg-yellow @endif flex items-center text-black group px-12 rounded-r-full py-4 max-mobile:p-4 max-mobile:rounded-r"
                   wire:navigate>
            ...
            @endrole
            <li class="mb-4">
                <form method="POST" action="/logout" class="text-black group pl-12 max-mobile:p-4">
                    @csrf
                    <button class="font-medium group-hover:text-red-500 flex items-center py-4" type="submit"
