<nav class="shrink-0 w-56 py-12 bg-yellow2 flex overflow-y-auto">
    <h2 class="sr-only">Navigation</h2>
    <ul class="flex flex-col">
        <li>
            <a href="{{ route('home') }}"
               class="@if (route('home') === url()->current() ) bg-yellow @endif flex items-center text-black group px-12 py-4 rounded-r-full"
        ...
        <li>
            <a href="{{ route('finances.general') }}"
               class="{{ request()->is('finances','finances'.'/*')?'bg-yellow':'' }} flex items-center text-black group px-12 rounded-r-full py-4"
        ...
        <li>
            <a href="{{ route('draw') }}"
               class="@if (route('draw') === url()->current() ) bg-yellow @endif flex items-center text-black group px-12 rounded-r-full py-4"
               title="Vers la page des détentes">
                <svg class="mr-2 stroke-black" xmlns="http://www.w3.org/2000/svg" width="25" height="25"
                     viewBox="0 0 32 32" fill="none">
                    <path
        ...
        <div class="mt-auto">
            ...
            <li>
                <a href="{{ route('settings') }}" title="Vers la page des paramètres"
                   class="@if (route('settings') === url()->current()) bg-yellow @endif flex items-center text-black group px-12 rounded-r-full py-4">
                    <svg class="mr-2 stroke-black" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                         viewBox="0 0 24 24" fill="none">
            ...
            @endrole
            <li class="mb-4">
                <form method="POST" action="/logout" class="text-black group pl-12">
                    @csrf
                    <button class="font-medium group-hover:text-red-500 flex items-center py-4" type="submit" title="Se déconnecter">
                        <svg class="mr-2 stroke-black group-hover:stroke-red-500" xmlns="http://www.w3.org/2000/svg"
                             width="25" height="25" viewBox="0 0 32 32" fill="none">
                            <path d="M21.75 10.75L27 16L21.75 21.25" stroke-width="2" stroke-linecap="round"
